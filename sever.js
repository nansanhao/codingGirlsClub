        ...
        req.models.User.find({usrEmail: newRecord.usrEmail}, function (err, user) {
            ...
            if (user.length == 0) {
                mailer({
                    ...
                    subject: '激活帐号',
                    text: `点击激活：http://47.94.147.137:8081/checkCode?mail=${newRecord.usrEmail}&psw=${newRecord.usrPassword}&id=${newRecord.id}`//接收激活请求的链接
                })
                res.send("ok")
                console.log("ok")
            }
    ...
    req.models.User.find({usrEmail:req.body.signEmail},function(err,user){
        ...
        if(user.length==0) {
            ...
            mailer({
                ...
                subject:'重置密码',
                text: `点击重置：http://47.94.147.137:8081/resetpass?mail=${req.body.signEmail}&password=${req.body.signPassword}`//接收激活请求的链接
            });
            res.send("ok")
        }
    })
